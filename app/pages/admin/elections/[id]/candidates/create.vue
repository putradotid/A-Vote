...
const { createCandidate, loading: isSubmitting, error: apiError } = useCandidateAdmin()
...
const loadElection = async () => {
  ...
  try {
    ...
    pageError.value = err instanceof Error ? err.message : 'Gagal memuat informasi pemilihan.'
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    await createCandidate(electionId.value, {
...
<template>
  <div>
    ...
    <div v-if="pageLoading" class="card p-12 text-center text-text-muted">
      ...
      <p class="text-body">Memuat formulir...</p>
    </div>

    <!-- Form Content -->
    <div v-else-if="election" class="max-w-3xl">
      <!-- Header -->
      <div class="mb-6 pb-6 border-b border-border">
        <div class="flex items-center gap-3">
          <h1 class="text-h2 font-bold text-text-primary">Tambah Kandidat / Paslon</h1>
      ...
      >
        <p class="font-semibold">Penambahan Paslon Tidak Diizinkan</p>
        <p class="mt-1">
          Pemilihan saat ini berstatus <strong>{{ election.computedState }}</strong>.
          Kandidat hanya dapat ditambahkan pada status <strong>DRAFT</strong> atau <strong>SCHEDULED</strong>.
        </p>
      </div>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="flex items-center justify-end gap-3 pt-4 border-t border-border">
            ...
            >
              ...
              ></span>
              <span>{{ isSubmitting ? 'Menyimpan...' : 'Simpan Paslon' }}</span>
            </button>
          </div>
        </form>
      </div>
